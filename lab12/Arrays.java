...
public class Arrays{
    ...
    public static int[][] raggedArray() {
        ...
        int stuff[][] = new int[(int) (Math.random()*11)+10][0]; //fills the first dimension of the array with a random number from 10 to 20
        for (int i = 0; i<stuff.length; i++) {
            //for loop runs in increments of 1 until it reaches the length of the first dimension
            stuff[i] = new int[(int) (Math.random()*11)+10]; //each member array will have a random number of members from 10 to 20
            for (int j = 0; j<stuff[i].length; j++) {
                //for loop runs in increments of 1 until it fills the length of the second dimension
                stuff[i][j] = (int) (Math.random()*2002)-1001; //fills each member array with random integers from -1000 to 1000
            }
        }
